...
| `pipeline.py` | Local glue: `process_pdf(path)` chains Steps 1→2→3 for local testing |
| `lambda_handler.py` | AWS Lambda entry point — S3 event → pipeline → DynamoDB write |
| `run_test.py` | Local test script (not deployed to Lambda) |
...
**Zip with 7-Zip, not PowerShell's `Compress-Archive`** — the native Windows zip tool can silently drop files with long nested paths (a real issue we hit with PyMuPDF's `_extra.so`):
...
- **Runtime settings → Handler**: `lambda_handler.lambda_handler`
